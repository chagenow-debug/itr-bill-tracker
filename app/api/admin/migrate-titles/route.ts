...
export async function GET() {
  try {
    ...
    // Get all bills with their current short_title
    const result = await query("SELECT id, short_title FROM bills");
    const bills = result.rows;
    console.log("[migrate-titles] Found", bills.length, "bills");
